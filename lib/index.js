...
class MailListener extends EventEmitter {
  constructor(options) {
    ...
    if (this.attachments) {
      if (!this.attachmentOptions.stream) {
        const directoryAttachments = options.attachmentOptions.directory;
        if ('string' !== typeof directoryAttachments) {
          const error = new Error('invalid attachments directory');
          throw error;
        }

        if (!directoryAttachments.endsWith('/')) {
          options.attachmentOptions.directory = `${directoryAttachments}/`;
        }
      }
    }
            ...
            message.on('body', (stream, info) => {
              ...
              parser.on('data', data => {
                if (data.type === 'text') {
                  ...
                  const {stream} = this.attachmentOptions;
                  if (stream) {
